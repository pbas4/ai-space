...
Require the planner's `planArtifact` descriptor and attach it as a conversation Markdown artifact. The structured plan remains the approval source of truth. The coordinator retains the planner's initial artifact and all previous revisions. Never use a SHA-256 value as the artifact title, filename, or content. Conversation artifacts must not modify the target repository. If the attachment is unavailable, report that gap and render the Markdown inline under its intended filename.
...
3. Present the resulting plan and wait for explicit approval before implementation.
   Publish the synthesized final plan as the next `artifactRevision` in a new conversation Markdown artifact, even when revision 1 came from RW CRM. Retain the planner's initial artifact and previous revisions. Name approval prompts by the readable filename and revision while keeping approval hashes internal.
